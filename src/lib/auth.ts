...
export async function verifySessionToken(token: string) {
  ...
  if (!payloadEncoded || !sigEncoded) {
    console.error("verifySessionToken: Missing payload or signature", { token });
    return null;
  }
  ...
  if (!valid) {
    console.error("verifySessionToken: Invalid signature", { token });
    return null;
  }
  ...
  try {
    ...
    if (!parsed.expiresAt || Date.now() > parsed.expiresAt) {
      console.error("verifySessionToken: Expired", { parsed });
      return null;
    }

    return parsed;
  } catch (err) {
    console.error("verifySessionToken: Parse error", err);
    return null;
  }
}
